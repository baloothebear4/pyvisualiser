...
float map(vec3 p){
    for( int i = 0; i<8; ++i){
        // This is the spin speed
        float t = iTime*(0.1+u_vu*0.1);   
        p.xz =rotate(p.xz,t);
        p.xy =rotate(p.xy,t*1.89);
        p.xz = abs(p.xz);
        // Make the fractal larger with bass. A smaller offset = larger object.
        p.xz -= (0.5);// - u_bass * 0.01);
    }
...
vec4 rm (vec3 ro, vec3 rd){
    ...
    for(float i =0.; i<64.; i++){
        ...
        float brightness = 400.0 - (u_bass * 20.0);
