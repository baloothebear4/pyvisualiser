...
class RenderTarget:
    ...
    def _build(self):
        ...
        if hasattr(self, 'depth_buffer') and self.depth_buffer: self.depth_buffer.release()
    ...
    def use(self):
        self.fbo.use()
        # Ensure viewport matches the target size (required for correct scaling of bloom/blur)
        self.ctx.viewport = (0, 0, self.texture.width, self.texture.height)

    def clear(self, colour=(0,0,0,0)):
        self.fbo.clear(
            red=colour[0], green=colour[1], blue=colour[2], alpha=colour[3],
            depth=1.0  # Clear depth to 1.0 (Far)
        )
...
class Compositor:
    ...
    def add_pre_pass(self, render_pass: RenderPass, at_start=False):
        if at_start:
            self.pre_passes.insert(0, render_pass)
        else:
    ...
    def clear_passes(self):
        """ Reset transient passes for a new frame """
        self.pre_passes = []
        self.passes = []
    ...
    def render_frame(self):
        ...
        self.main_target.clear(colour=(bg[0], bg[1], bg[2], 0.0))
        
        # Reset Global GPU State for new frame
        self.context.ctx.disable(moderngl.DEPTH_TEST)
        self.context.ctx.depth_mask = False
        ...
        # Execute all registered passes (GeometryPass and then GlowPass)
        for p in self.passes:
            p.render()
        ...
        if self.debug_view == 'glow':
            self.final_blit_pass.render(self.glow_buffer.read)
        elif self.debug_view == 'bg':
            self.final_blit_pass.render(self.main_target)
        elif self.debug_view == 'ui':
            self.final_blit_pass.render(self.main_target)
        else:
