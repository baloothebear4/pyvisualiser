...
class BackgroundLighting:
    ...
    def update(self, audio_processor):
        ...
        # 3. Audio Stats (Direct pass through to shader)
        if hasattr(audio_processor, 'vu'):
            self.audio_vu = audio_processor.vu['mono'] * 1.0
            self.bass = getattr(audio_processor, 'bass', 0.0)
            self.treble = getattr(audio_processor, 'treble', 0.0)
            # Capture full audio analysis state
            self.audio_analysis = audio_processor.get_analysis() if hasattr(audio_processor, 'get_analysis') else {}
...
class BackgroundRenderPass(RenderPass):
    ...
    FRAGMENT_TEMPLATE = """
        ...
        // Audio Analysis (Standardised across visualiser shaders)
        ...
        // --- INJECTED PATTERN CODE START ---
        %s
        // --- INJECTED PATTERN CODE END ---

        void main() {
            // 0. Base colour
            vec4 colour = vec4(u_colour, u_colour_opacity);
            
            // 1. Dynamic Pattern (Integrated at the bottom-most graphical layer)
            if (u_use_pattern) {
                vec2 screen_coords = u_frame_offset + v_uv * u_frame_size;
                vec4 pattern_col = run_pattern(u_screen_size, screen_coords);
                colour.rgb = mix(colour.rgb, pattern_col.rgb, pattern_col.a);
            }
            ...
            // 3. Noise Layer (Film Grain)
            if (u_noise_strength > 0.0) {
                float noise_val = random(v_uv + iTime * 0.1);
                colour.rgb += (noise_val - 0.5) * u_noise_strength;
            }
            
            // 4. Ambient Glow (Radial wash from bottom-center)
            if (u_ambient_opacity > 0.0) {
                float dist = distance(v_uv, vec2(0.5, 0.0));
            ...
            // 5. Reactive Glow (Pulsing from center)
            if (u_reactive_intensity > 0.0) {
                ...
                colour.a = max(colour.a, glow * u_reactive_intensity);
            }
            ...
            if (u_starfield_density > 0.0) {
                for (float i = 1.0; i <= 3.0; i++) {
                    ...
                    vec2 shift = vec2(iTime * speed, 0.0);
            ...
            if (u_cloud_opacity > 0.0) {
                ...
                float brightness = pow(blend, 1.5) * 0.6 + 0.15;
                brightness += pow(u_audio_vu, 0.5) * 0.05;
                vec3 col1 = max(u_colour * 4, vec3(0.01));
                vec3 col2 = u_colour * 25 + 0.05;
                vec3 cloud_rgb = mix(col1, col2, pow(blend, 1.4));
                colour.rgb = mix(colour.rgb, cloud_rgb * brightness * vignette, u_cloud_opacity);
            }

            // 10. Panel Edge Lighting
            if (u_edge_intensity > 0.0) {
                float edge = edge_mask(v_uv);
                float audio_boost = 1.0 + pow(u_audio_vu, 0.5) * u_edge_audio_reactivity;
                vec3 edge_col = mix(u_colour, vec3(1.0), 0.7) * 1.0 + 0.05;
    ...
    def __init__(self, context: RenderContext, style: BackgroundStyle, surface: BackgroundSurface, lighting: BackgroundLighting):
        ...
        shader_name = style.shader
        if shader_name is True: shader_name = 'balatro'
        ...
        if shader_name:
            try:
                ...
                for ext in ['.glsl', '.frag.glsl']:
                    try:
                        p = get_asset_path('shaders', f"{shader_name}{ext}")
                        if os.path.exists(p):
                            path = p
                            break
                    except: pass
                ...
                if path:
                    ...
                    # 2. Transform the external shader
...
"""
                    ...
                    print(f"BackgroundRenderPass> Warning: Shader '{shader_name}' not found.")
            except Exception as e:
                print(f"BackgroundRenderPass> Error loading shader '{shader_name}': {e}")

        # Fallback if no pattern loaded
        if not pattern_code:
            pattern_code = "vec4 run_pattern(vec2 s, vec2 c) { return vec4(0.0); }"

        self.prog = self.ctx.program(
            vertex_shader=self.DEFAULT_VERT,
    ...
    def render(self, **kwargs):
        ...
        if 'iResolution' in self.prog:
             self.prog['iResolution'].value = (float(self.context.width), float(self.context.height))

        if 'u_screen_size' in self.prog:
            self.prog['u_screen_size'].value = (float(self.context.width), float(self.context.height))
        
        if self.viewport:
            if 'u_frame_offset' in self.prog:
                self.prog['u_frame_offset'].value = (float(self.viewport[0]), float(self.viewport[1]))
            if 'u_frame_size' in self.prog:
                self.prog['u_frame_size'].value = (float(self.viewport[2]), float(self.viewport[3]))

        # Pass all audio analysis uniforms
        if 'u_vu' in self.prog:
            self.prog['u_vu'].value = self.lighting.audio_vu
        if 'u_bass' in self.prog:
            self.prog['u_bass'].value = self.lighting.bass
        if 'u_treble' in self.prog:
            self.prog['u_treble'].value = self.lighting.treble
        
        # Additional spectral/beat analysis
        if hasattr(self.lighting, 'audio_analysis'):
            for key, value in self.lighting.audio_analysis.items():
                u_key = f"u_{key}"
                if u_key in self.prog:
                    if isinstance(value, bool):
                        self.prog[u_key].value = int(value)
                    else:
                        self.prog[u_key].value = float(value)
        ...
        if self.style.shader:
            self.prog['u_use_pattern'].value = True
        else:
            self.prog['u_use_pattern'].value = False

        # Disable depth writing for backgrounds
        self.ctx.disable(moderngl.DEPTH_TEST)
...
class BackgroundBase:
    ...
    def draw(self):
        # Update lighting state based on audio before drawing
        self.update()
