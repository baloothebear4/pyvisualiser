...
from pyvisualiser.core.backgrounds import BackgroundRenderPass, BackgroundStyle, BackgroundSurface, BackgroundLighting, BackgroundBase
from pyvisualiser.styles.styles import AmbientGlowStyle
...
class GreenQuadPass(RenderPass):
    """Draws a green quad in the top-right corner."""
    def __init__(self, context):
        super().__init__(context)
        self.prog = self.ctx.program(
            vertex_shader="""
                ...
                in vec2 in_vert;
                void main() {
                    // Scale and translate to top-right
                    gl_Position = vec4(in_vert * 0.3 + vec2(0.5, 0.5), 0.0, 1.0);
                }
            """,
            fragment_shader="""
                #version 330
...
def run_test():
    ...
    # We add multiple passes to verify they stack correctly
