...
class RotatedScreen(FloatLayout):
    ...
    def add_widget(self, widget, index=0, **kwargs):
        """
        Override the add_widget method to apply rotation to all widgets added.
        """
        # We only apply the rotation to the main content box
        if widget == self.content_box:
        ...
        super(RotatedScreen, self).add_widget(widget, index, **kwargs)
