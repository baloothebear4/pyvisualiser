...
# The most reliable way to handle this is to set the rotation
# at the config level before the application starts.
...
class RotationTestApp(App):
    ...
    def on_window_size(self, window, size):
        ...
        # We are now only setting the rotation origin to keep the content centered
        self.root.children[0].pos = (Window.width / 2 - self.root.children[0].width / 2, Window.height / 2 - self.root.children[0].height / 2)
        self.rot.origin = (self.root.children[0].center_x, self.root.children[0].center_y)
