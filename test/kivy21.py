...
class RotatedRoot(Widget):
    ...
    def _update_transform(self, *args):
        ...
        with self.canvas.before:
            PushMatrix()
            # Rotate -90° (landscape, ribbon at left)
            Translate(0, self.height)
            self.rot_angle = -90
            Rotate(origin=(0, 0), angle=self.rot_angle)
        with self.canvas.after:
            PopMatrix()

    # --- Touch remapping ---
    def _rotate_point(self, x, y):
        """Rotate a touch coordinate -90° around origin (0,0) then translate."""
        w, h = self.width, self.height
        # Input is portrait: (0..400, 0..1280)
        # We want landscape: (0..1280, 0..400)

        # Apply -90° rotation: (x, y) -> (y, w - x)
        new_x = y
        new_y = w - x
        return new_x, new_y
