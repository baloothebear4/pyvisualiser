...
class Background:
    ...
    def __init__(self, frame, background=None):
        ...
        if background is None:
            ...
            # New Engine Path: If a BackgroundStyle is passed, use the new engine.
            if hasattr(self.frame.platform, 'compositor'):
                self.engine = BackgroundBase(self.frame.platform, background)
                # Set a flag so per_frame_update is true, this is needed for Frame.draw_background
                self.background = {'per_frame_update': True}
            else:
                print("Background WARNING: BackgroundStyle provided but not in OpenGL mode. Background will be blank.")
        ...
        elif isinstance(background, str) and background.lower().endswith(('.jpg', '.png')):
    ...
    def is_opaque(self):
        ...
        if self.background is None:
            ...
            if 'opacity' not in self.background: 
                return True
            else:
    ...
    def draw(self, perform_update=True):
        # --- New Engine Path ---
        if self.engine:
            # Set the viewport from the frame's absolute coordinates
            # print("rendered background draw")
            x, y, w, h = self.frame.abs_background()
            # Convert Pygame top-left Y to OpenGL bottom-left Y
            gl_y = self.frame.platform.H - (y + h)
            ...
            # Update engine with audio data
            audio_data = {'mono': 0.0, 'peak_mono': 0.0}
            if hasattr(self.frame.platform, 'vu'):
                 audio_data = {
                     'mono': self.frame.platform.vu.get('mono', 0.0),
                     'peak_mono': self.frame.platform.peak.get('mono', 0.0) if hasattr(self.frame.platform, 'peak') else 0.0
                 }
            self.engine.update(audio_data)
            
            # Add the render pass to the compositor for this frame
            if hasattr(self.frame.platform, 'compositor'):
                # print("rendered background compositor")
                self.frame.platform.compositor.add_pre_pass(self.engine.get_render_pass())
            return
        ...
        self.prog = self.ctx.program(
            ...
            fragment_shader="""
                ...
                void main() {
                    ...
                    // f_color = color;
                    f_color = vec4(v_uv, 0.0, 1.0); // DEBUG: Output UVs as color (Red/Green gradient)
                }
            """
        )
        
        # Switch to shared context quad buffer (known good)
        self.quad_vao = self.ctx.simple_vertex_array(
            self.prog, self.context.get_quad_buffer(), 'in_vert', 'in_uv'
        )

    def render(self, target=None, **kwargs):
        # print(f"BG Pass: FBO {self.ctx.fbo.glo}, Viewport {self.ctx.viewport}, Scissor {self.ctx.scissor}")
        
        # 1. State Setup: Force FBO and Mask to ensure we can write
        if target:
            target.use()
        else:
            self.ctx.fbo.use()
        ...
        original_scissor = None
        if self.viewport:
            original_viewport = self.ctx.viewport
            original_scissor = self.ctx.scissor
            self.ctx.viewport = self.viewport
            self.ctx.scissor = self.viewport # Restrict clear/draw to this tile
        # self.ctx.scissor = None # Force disable scissor for debug
        ...
        # print(f"  BackgroundRenderPass> Clearing FBO {self.ctx.fbo.glo} to MAGENTA")
        ...
        self.prog['u_time'].value = (time.time() - self.start_time) % 1000.0

        # Resolve colors from theme
        def get_rgb(color_name):
            c = self.colours.get(color_name)
            if not c: return (0.0, 0.0, 0.0) # Safeguard against missing colors
            return (c[0] / 255.0, c[1] / 255.0, c[2] / 255.0)

        # Set uniforms from style and state objects
        base_color_val = get_rgb(self.style.base_color)
        self.prog['u_base_color'].value = base_color_val
        
        s_vignette = self.style.vignette
        vignette_strength_val = s_vignette.strength
        self.prog['u_vignette_strength'].value = float(vignette_strength_val)
        self.prog['u_vignette_radius'].value = float(s_vignette.radius)
        self.prog['u_vignette_softness'].value = float(s_vignette.softness)

        s_noise = self.style.noise
        noise_strength_val = s_noise.strength
        self.prog['u_noise_strength'].value = float(noise_strength_val)
        self.prog['u_noise_speed'].value = float(s_noise.speed)

        s_ambient = self.style.ambient_glow
        ambient_opacity_val = s_ambient.opacity
        self.prog['u_ambient_glow_color'].value = get_rgb(s_ambient.color)
        self.prog['u_ambient_glow_opacity'].value = float(ambient_opacity_val)
        self.prog['u_ambient_glow_radius'].value = float(s_ambient.radius)
        self.prog['u_ambient_glow_softness'].value = float(s_ambient.softness)

        reactive_intensity_val = self.lighting.reactive_intensity
        self.prog['u_reactive_glow_color'].value = get_rgb(self.style.reactive_glow.color)
        self.prog['u_reactive_intensity'].value = float(reactive_intensity_val)

        peak_intensity_val = self.lighting.peak_intensity
        self.prog['u_peak_accent_color'].value = get_rgb(self.style.peak_accent.color)
        self.prog['u_peak_intensity'].value = float(peak_intensity_val)

        tex = self.surface.get_texture(self.ctx)
        has_texture_val = False
        if tex:
            tex.use(location=0)
        ...
        # print(f"  Has Texture: {has_texture_val}, Base Color: {base_color_val}")
        # --- END DEBUGGING ---
        ...
        if original_scissor is not None:
            self.ctx.scissor = original_scissor
        else:
