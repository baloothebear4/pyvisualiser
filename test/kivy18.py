# kivy_landscape_rotated.py
from kivy.config import Config

# Keep the screen's native portrait resolution
...
class RotatedRoot(Widget):
    """Container that rotates its children 90° clockwise to simulate landscape."""
    ...
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            ...
            Translate(self.height, 0)
            # Rotate 90 degrees clockwise
            self.rot = Rotate(origin=(0, 0), angle=-90)
        with self.canvas.after:
            PopMatrix()

        # Use a BoxLayout as the actual app content
        self.layout = TestLayout()
        self.add_widget(self.layout)

        # Update transform when size changes
    ...
    def _update_transform(self, *args):
        ...
        with self.canvas.before:
            PushMatrix()
            # Translate to new height because rotation swaps axes
            Translate(self.height, 0)
            self.rot = Rotate(origin=(0, 0), angle=-90)
