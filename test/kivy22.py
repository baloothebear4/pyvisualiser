...
# "ccw" → ribbon on left   (UI rotated -90°)
# "cw"  → ribbon on right  (UI rotated +90°)
# -------------------------
ORIENTATION = "cw"   # change to "ccw" if needed
...
class RotatedRoot(Widget):
    ...
    def _update_transform(self, *args):
        ...
        with self.canvas.before:
            PushMatrix()
            if ORIENTATION == "ccw":
                # Rotate -90° (landscape, ribbon left)
                Translate(0, self.height)
                Rotate(origin=(0, 0), angle=-90)
            else:
                # Rotate +90° (landscape, ribbon right)
