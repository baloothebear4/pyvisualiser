...
class RotatedScreen(RelativeLayout):
    ...
    def update_transformation(self, instance, size):
        ...
        if self.is_rotated:
            # When rotated, the canvas becomes landscape
            # We translate the content up and to the left to keep it in view
            self.trans.x = window_height
            self.trans.y = 0
            
            # The rotation is now applied relative to the top-left corner
            self.rot.origin = (0, 0)
            self.rot.angle = -90
