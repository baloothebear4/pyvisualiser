# kivy_landscape_touch_sliderfix2.py
...
Config.set('input', 'mouse', 'mouse,disable_multitouch')  # disable rotated mouse pointer
Config.set('modules', 'cursor', '')  # hide system cursor
...
class FixedSlider(Slider):
    """Slider with corrected drag direction in rotated landscape mode."""

    def on_touch_move(self, touch):
        if self.collide_point(*touch.pos):
            # force motion along x-axis even if y is moving
            touch.x, touch.y = touch.pos[0], self.center_y
        return super().on_touch_move(touch)
