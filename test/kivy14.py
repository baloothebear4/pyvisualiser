...
class RotatedScreen(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # Bind the window size event to update the transformation
    ...
    def update_transformation(self, instance, size):
        ...
        # The rotation origin is the center of the screen
        self.rot.origin = (window_width / 2, window_height / 2)

        if self.is_rotated:
            # We want to rotate the content to landscape
            self.rot.angle = -90
            
            # The translation needs to correct for the center of the rotated box
            self.trans.x = window_width
            self.trans.y = 0
...
class RotatedApp(App):
    def build(self):
        ...
        # Create the content box that will be placed inside the container
        self.content_box = BoxLayout(orientation='vertical', padding=100, spacing=20)
        self.content_box.size_hint = (None, None)
        self.content_box.size = (1280, 400) # This is the landscape size we want
