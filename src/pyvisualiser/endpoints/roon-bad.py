...
class RoonMetaData:
    ...
    @property
    def zone(self):
        if not self.roon or not self._target_zone_name or self._target_zone_name == 'None':
            return None
        try:
            # Try exact match first
            z = self.roon.zone_by_name(self._target_zone_name)
            if z: return z
            # Inclusive search for grouped zones
            return next((zone for zone in self.roon.zones.values() 
                        if self._target_zone_name.lower() in zone.get('display_name', '').lower()), None)
        except Exception as e:
            print(f"RoonMetaData.zone ERROR: {e}")
            return None
    ...
    def set_target_zone(self, name):
        ...
        try:
            ...
            print(f"RoonMetaData.set_target_zone ERROR: {e}")
            self._target_zone_name = 'None'
    ...
    def update(self, changed_zoneids):
        ...
        # Inclusive check: loop through changed IDs and see if any display name contains target name
        matched_zone = None
        for zid in changed_zoneids:
            if zid in self.roon.zones:
                zname = self.roon.zones[zid].get('display_name', '')
                if self.target_zone_name.lower() in zname.lower():
                    matched_zone = self.roon.zones[zid]
                    self._target_zone = zid # Update stored ID as it changes with groups
                    break

        if matched_zone:
            # print(f"ZoneMetadata.update> Match Found! Target: '{self.target_zone_name}' | Zone Name: '{matched_zone['display_name']}'")
            for key in RoonMetaData.ZONE_KEYS:
                if key in matched_zone:
                    self._metadata[key] = matched_zone[key]

            self.track_changed  = self.track != old_track
            self.play_changed   = self.play_status != old_play_status
            if self.track_changed:
                print("ZoneMetadata.update> now playing: Zone %s Track %s > Artist %s Album %s" % (matched_zone['display_name'], self.track, self.artist, self.album))
        else:
...
class Roon(RoonMetaData):
    def __init__(self, events, target_name='MacViz', maxwh=(400,400)):
        self.events = events
        self.image_cache = {}
        ...
        self.appinfo = {
            ...
            "email": "baloothebear4@gmail.com" }

        try:
            with open("my_core_id_file", "r") as f:
                core_id = f.read().strip()
            with open("my_token_file", "r") as f:
                token = f.read().strip()
            if not core_id or not token:
                raise ValueError("Core ID or Token is empty")
        ...
        if not core_id:
            ...
            print("Roon.__init__>> failed to initialise: No core ID found.")
            return
        ...
        if server is None:
            RoonMetaData.__init__(self, None, maxwh)
            print("Roon.__init__>> failed to initialise: Roon server discovery returned None.")
            return
        ...
        try:
            ...
            RoonMetaData.__init__(self, None, maxwh)
            print(f"Roon.__init__>> failed to initialise: {e}") 
    ...
    def metadata_stop(self):
        if self.roon:
            try:
                ...
                print(f"Roon.metadata_stop ERROR: {e}")

    def startup(self):
        print("Roon.startup>> Discover Roon servers and connect")
        ...
        if not servers:
            print("Roon.startup>> No Roon servers discovered on the network.")
            return None, None
        ...
        while len(auth_api) == 0:
            print("Roon.startup>> Waiting for authorisation on Roon display -> settings/extensions", )
            time.sleep(1)
            auth_api = [api for api in apis if api.token is not None]

        api = auth_api[0]

        print("Roon.startup>> Got authorisation", api)
        print(api.host)
        print(api.core_name)
        print(api.core_id)

        print("Roon.startup>> Shutdown apis")
        for a in apis:
            a.stop()
...
def RoonAction(e):
    print(f'RoonAction> event {e}')
    if 'roon' in globals() and roon.roon is not None:
...
if __name__ == "__main__":
    ...
    print("Starting Roon metadata test...")
    ...
    try:
        events.Metadata += RoonAction     # respond to metadata changes
        last_print = 0
        while True:
            ...
            if now - last_print >= 1.0:
                if roon.roon is not None:
                    print(f"[{time.strftime('%H:%M:%S')}] Zone: '{roon.zone_name}' | Artist: '{roon.artist}' | Track: '{roon.track}' | Album: '{roon.album}' | Progress: {roon.elapsed:.1f}/{roon.duration:.1f}s ({roon.elapsedpc*100:.1f}%) | State: {roon.play_status}")
                else:
                    print(f"[{time.strftime('%H:%M:%S')}] Roon is not connected (failed to initialise or discovery failed).")
                last_print = now
