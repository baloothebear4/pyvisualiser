...
class TextFrame(Frame):
    """
        ...
        - Y is the y scaler
    """
    def __init__(self, bounds, platform, display, Valign='top', scalers=(1.0, 1.0), text='Default Text', Halign='centre', boxV='middle', boxH='centre', fontmax=0, reset=False,theme='std'):
        Frame.__init__(self, bounds=bounds, platform=platform, display=display, scalers=scalers, Valign=Valign, Halign=Halign)
        self.text     = Text(display, self.coords, text, fontmax=fontmax, reset=reset, Valign=boxV, Halign=boxH, theme=theme, colour_index='foreground')
...
class PlayProgressFrame(Frame):
    ...
    def __init__(self, bounds, platform, display, scalers=(1.0, 1.0), Valign='bottom', Halign='left', barsize_pc=0.5, theme='std', flip=False, \
        ...
        Frame.__init__(self, platform=platform, bounds=bounds, display=display, scalers=scalers, Valign=Valign, Halign=Halign)
        self.barw           = self.w * barsize_pc if orient == 'vert' else self.h * barsize_pc   # width of the bar

        text = " 22:22 "
        self.elapsed     = Text(display, self.coords, fontmax=self.barw, text=text, reset=True, Valign='middle', Halign='left', theme=theme, colour_index='mid')
        self.remaining   = Text(display, self.coords, fontmax=self.barw, text=text, reset=True, Valign='middle', Halign='right', theme=theme, colour_index='mid')
        ...
        self.boxbar      = Box(display, self.coords, Valign='middle', theme=theme, box=box, radius=10)
...
class AlbumArtFrame(Frame):
    def __init__(self, bounds, platform, display, scalers=(1.0,1.0), Valign='top', Halign='centre'):
        Frame.__init__(self, bounds, platform, display, scalers=scalers, Valign=Valign, Halign=Halign)
        self.image_container = Image(display, self.coords)  # make square
        self.outline = OutlineFrame(bounds, platform, display, scalers=scalers, Valign=Valign, Halign=Halign)
...
class MetaDataFrame(Frame):
    SHOW = {'artist': {'colour':'foreground', 'Valign': 'bottom'}, \
            'track': {'colour':'light', 'Valign': 'top'}, \
            'album': {'colour':'mid', 'Valign': 'middle'} }

    def __init__(self, bounds, platform, display, scalers=(1.0,1.0), Valign='top', Halign='centre',theme='std',show=SHOW):
        Frame.__init__(self, bounds, platform, display, scalers=scalers, Halign=Halign, Valign=Valign)
        # Resize new Text containers each time this is executed
        self.show = show
        if 'track' in self.show:  self.track_container     = TextFrame(self.coords, platform, display, scalers=(1.0, 0.33), Valign=self.show['track']['Valign'], Halign='centre', reset=True, theme=theme)
        if 'album' in self.show:  self.album_container     = TextFrame(self.coords, platform, display, scalers=(1.0, 0.33), Valign=self.show['album']['Valign'], Halign='centre', reset=True, theme=theme)
        if 'artist' in self.show: self.artist_container    = TextFrame(self.coords, platform, display, scalers=(1.0, 0.33), Valign=self.show['artist']['Valign'], Halign='centre', reset=True, theme=theme)
        # self.back = Lightback(display, self.coords)
...
class VUMeter(Frame):
    ...
    def __init__(self, bounds, platform, display, channel, scalers=(1.0, 1.0), Valign='bottom', Halign='left', peakmeter=False, \
        ...
        self.marks   = marks
        Frame.__init__(self, platform=platform, bounds=bounds, display=display, scalers=scalers, Valign=Valign, Halign=channel)
        ...
        if bgdimage is not None:
            ...
            self.bgdimage   = Image(display, self.coords, Valign='middle', Halign='centre', path=self.path )
            # print("VUeter.__init__> setup background images")
        else:
            self.path        = None
            radius           = self.h*(0.5-pivot)
            self.scales      = Text(display, self.coords, fontmax=self.h*fonth, endstops=endstops, centre_offset=pivot, \
                                    radius=radius*scaleslen, theme=theme )
            self.dB          = Text(display, self.coords, fontmax=self.h*fonth*2, text=annotate['text'], Valign=annotate['Valign'], colour_index=annotate['colour'], reset=True, theme=theme)
            self.ticks       = Line(display, self.coords, width=tick_w, endstops=endstops, tick_pc=tick_pc, centre_offset=pivot, radius=radius*ticklen, theme=theme )
            self.arclines    = []
            for rad_pc, arc in arcs.items():
                self.arclines.append(Line(display, self.coords, width=arc['width'], colour_index=arc['colour'], endstops=endstops, tick_pc=tick_pc, centre_offset=pivot, radius=radius*rad_pc, theme=theme ))

        # setup the needle
        radius           = self.h*(0.5-pivot)
        self.VU          = VU(platform, channel, decay=decay, smooth=smooth)
        self.needle      = Line(display, self.coords, width=needle['width'], tick_pc=needle['radius_pc'], centre_offset=pivot, endstops=endstops, radius=radius*needle['length'], theme=theme, colour_index=needle['colour'])
        self.peak        = Line(display, self.coords, width=needle['width'], tick_pc=needle['radius_pc'], centre_offset=pivot, endstops=endstops, radius=radius*needle['length'], theme=theme, colour_index='alert')
...
class VUMeterFrame1(Frame):
    ...
    def __init__(self, bounds, platform, display, scalers=(1.0, 1.0), Valign='middle', Halign='centre'):
        ...
        self += VUMeter(self.coords, platform, display, 'left', scalers=(0.5, 1.0), Valign='bottom', Halign='left', arcs={}, endstops=ENDSTOPS, needle=NEEDLE)
        self += VUMeter(self.coords, platform, display, 'right', scalers=(0.5, 1.0), Valign='bottom', Halign='right', arcs={}, endstops=ENDSTOPS, needle=NEEDLE)
...
class VUMeterFrame2(Frame):
    ...
    def __init__(self, bounds, platform, display, scalers=(1.0, 1.0), Valign='middle', Halign='centre'):
        ...
        self += VUMeter(self.coords, platform, display, 'left', scalers=(0.5, 1.0), Valign='bottom', Halign='left', \
                        pivot=-0.4, endstops=ENDSTOPS, peakmeter=True, needle=NEEDLE, marks=MARKS, arcs=ARCS, annotate=ANNOTATE, smooth=SMOOTH)
        self += VUMeter(self.coords, platform, display, 'right', scalers=(0.5, 1.0), Valign='bottom', Halign='right', \
                        pivot=-0.4, endstops=ENDSTOPS, peakmeter=True, needle=NEEDLE, marks=MARKS, arcs=ARCS, annotate=ANNOTATE, smooth=SMOOTH)

class VUMeterFrame3(Frame):
    """ 270 speedo dial type VU - colourful """
    def __init__(self, bounds, platform, display, scalers=(1.0, 1.0), Valign='bottom', Halign='left'):
        Frame.__init__(self, bounds=bounds, platform=platform, display=display, scalers=scalers, Valign=Valign, Halign=Halign)
        TICK_W    = 3
        ARCLEN    = 0.70
        MARKS     = {0.0: {'text':'0', 'width': TICK_W, 'colour': 'mid'},
...
class VUMeterFrame4(Frame):
    ...
    def __init__(self, bounds, platform, display, scalers=(1.0, 1.0), Valign='middle', Halign='centre'):
        ...
        ANNOTATE  = { 'Valign':'middle', 'text':'dB', 'colour':'mid' }
        self += VUMeter(self.coords, platform, display, 'left', scalers=(0.5, 1.0), Valign='bottom', Halign='left', annotate=ANNOTATE,\
                        pivot=-0.7, endstops=(3*PI/4, 5*PI/4), tick_pc=TICK_PC, peakmeter=False, needle=NEEDLE, marks=MARKS, arcs=ARCS)
        self += VUMeter(self.coords, platform, display, 'right', scalers=(0.5, 1.0), Valign='bottom', Halign='right', annotate=ANNOTATE,\
                        pivot=-0.7, endstops=(3*PI/4, 5*PI/4), tick_pc=TICK_PC, peakmeter=False, needle=NEEDLE, marks=MARKS, arcs=ARCS)

class VUMeterImageFrame(Frame):
    """ Image background based class - the """
    def __init__(self, bounds, platform, display, type=None, scalers=(1.0, 1.0), Valign='middle', Halign='centre'):
        Frame.__init__(self, bounds=bounds, platform=platform, display=display, scalers=scalers, Valign=Valign, Halign=Halign)
        ...
        meter = METERS[type]
        self += VUMeter(self.coords, platform, display, 'left', scalers=(0.5, 1.0), Valign='bottom', Halign='left', theme=meter['theme'],\
                        pivot=meter['pivot'], endstops=meter['endstops'], needle=meter['needle'], bgdimage=meter['file'])
        self += VUMeter(self.coords, platform, display, 'right', scalers=(0.5, 1.0), Valign='bottom', Halign='left', theme=meter['theme'],\
                        pivot=meter['pivot'], endstops=meter['endstops'], needle=meter['needle'], bgdimage=meter['file'])
...
class VUFrame(Frame):
    ...
    """
    def __init__(self, bounds, platform, display, channel, scale=(1.0, 1.0), V='bottom', H='left', barsize_pc=0.7, theme='std', flip=False, \
        ...
        Frame.__init__(self, platform=platform, bounds=bounds, display=display, scalers=scale, Valign=V, Halign=H)
        self.barw           = self.w * barsize_pc if orient == 'vert' else self.h * barsize_pc   # width of the bar
        box = (self.barw, self.h) if orient == 'vert' else (self.w, self.barw)
        self.bar = Bar(display, self.coords, Valign='middle', box_size=box, led_h=led_h, led_gap=led_gap, peak_h=peak_h, theme=theme, flip=flip, radius=radius, tip=tip, orient=orient)
...
class VU2chFrame(Frame):
    def __init__(self, bounds, platform, display, scalers, Halign='left', Valign='middle', orient='vert', flip=False, led_h=5, led_gap=1,barsize_pc=0.7, theme='std'):
        Frame.__init__(self, bounds, platform, display, scalers=scalers, Halign=Halign, Valign=Valign)
        # def VUVFrame(self, bounds, platform, display, channel, scale=(1.0, 1.0), V='bottom', H='left', barsize_pc=0.7, theme='std', flip=False, \
        #                 led_h=5, led_gap=1, peak_h=1, col_mode='h', radius=0, barw_min=10, barw_max=200, tip=False, decay=DECAY):
        if orient=='horz':
            self += VUFrame(self.coords, platform, display, 'left',  V='top', scale=(1.0, 0.5), orient=orient,flip=flip,  )
            self += VUFrame(self.coords, platform, display, 'right', V='bottom', scale=(1.0, 0.5), orient=orient,flip=flip )
        else:     # Vertical
            self += VUFrame(self.coords, platform, display, 'left', H='left', scale=(0.5, 1.0), orient='vert',flip=flip,led_h=led_h, led_gap=led_gap,barsize_pc=barsize_pc, theme=theme )
            self += VUFrame(self.coords, platform, display, 'right', H='right', scale=(0.5, 1.0), orient='vert',flip=flip,led_h=led_h, led_gap=led_gap,barsize_pc=barsize_pc, theme=theme )
        # self += OutlineFrame(self.coords, platform, display)
        self.check()

class VUFlipFrame(Frame):
    def __init__(self, bounds, platform, display, scalers, Halign='left', Valign='middle', orient='vert', flip=False,theme='std'):
        Frame.__init__(self, bounds, platform, display, scalers=scalers, Halign=Halign, Valign=Valign)
        # def VUVFrame(self, bounds, platform, display, channel, scale=(1.0, 1.0), V='bottom', H='left', barsize_pc=0.7, theme='std', flip=False, \
        #                 led_h=5, led_gap=1, peak_h=1, col_mode='h', radius=0, barw_min=10, barw_max=200, tip=False, decay=DECAY):
        if orient=='horz':
            self += VUFrame(self.coords, platform, display, 'left',  V='middle', H='left', scale=(0.5, 0.5), orient=orient,flip=True, tip=False,theme=theme )
            self += VUFrame(self.coords, platform, display, 'right', V='middle', H='right', scale=(0.5,0.5), orient=orient,flip=False, tip=False,theme=theme )
        else:     # Vertical
            self += VUFrame(self.coords, platform, display, 'left', H='left', V='top', scale=(0.5, 0.5), orient='vert',flip=False,theme=theme )
            self += VUFrame(self.coords, platform, display, 'right', H='left', V='bottom', scale=(0.5, 0.5), orient='vert',flip=True,theme=theme )
        # self += OutlineFrame(self.coords, platform, display)
        self.check()


class VUHorzFrame(Frame):
    def __init__(self, bounds, platform, display, channel, scalers, V, H, tip=False,theme='std'):
        Frame.__init__(self, bounds, platform, display, scalers, V, H)
        # def VUVFrame(self, bounds, platform, display, channel, scale=(1.0, 1.0), V='bottom', H='left', barsize_pc=0.7, theme='std', flip=False, \
        #                 led_h=5, led_gap=1, peak_h=1, col_mode='h', radius=0, barw_min=10, barw_max=200, tip=False, decay=DECAY):
        self += VUFrame(self.coords, platform, display, channel,  V='middle', H='right', scale=(0.90, 0.8), orient='horz', barsize_pc=0.8, led_gap=0,tip=tip, theme=theme )
        # def __init__(self, bounds, platform, display, V, Y, text='Default Text', X=1.0, H='centre', fontmax=0):
        channel_text = ' L' if channel=='left' else ' R'
      # def __init__(self, bounds, platform, display, Valign='top', scalers=(1.0, 1.0), text='Default Text', Halign='centre', fontmax=0):
        self += TextFrame(self.coords, platform, display, Valign='middle', Halign='left', scalers=(0.1, 0.2), text=channel_text, boxV='middle', boxH='left', theme=theme)

class VU2chHorzFrame(Frame):
    def __init__(self, bounds, platform, display, scalers, Valign='bottom', Halign='left',tip=False, theme='std'):
        # def __init__(self, bounds, platform=None, display=None, scalers=[1.0,1.0], Valign='bottom', Halign='left'):
        Frame.__init__(self, bounds, platform, display, scalers=scalers, Valign=Valign, Halign=Halign)
        # def VUVFrame(self, bounds, platform, display, channel, scale=(1.0, 1.0), V='bottom', H='left', barsize_pc=0.7, theme='std', flip=False, \
        #                 led_h=5, led_gap=1, peak_h=1, col_mode='h', radius=0, barw_min=10, barw_max=200, tip=False, decay=DECAY):
        # self += VUHorzFrame(self.coords, platform, display, 'left',  scalers=(0.5,1.0), V='middle' , H='left', flip=True )
        self += VUHorzFrame(self.coords, platform, display, 'left',  scalers=(1.0, 0.5), V='top' , H='left',tip=tip, theme=theme )
        self += VUHorzFrame(self.coords, platform, display, 'right', scalers=(1.0, 0.5), V='bottom' , H='left',tip=tip, theme=theme )
        # self += VUHorzFrame(self.coords, platform, display, 'right', scalers=(0.5,1.0), V='middle' , H='left' )

"""
Outline Frames
"""
class OutlineFrame(Frame):
    def __init__(self, bounds, platform, display, scalers, Valign='bottom', Halign='left', theme='std', width=4):
        Frame.__init__(self, bounds, platform, display, scalers=scalers, Valign=Valign, Halign=Halign)
        self.out     = Box(display, self.coords, self.wh, width=width, theme=theme)
...
class Spectrum2chFrame(Frame): #""" Vert split - L/R """
    def __init__(self, bounds, platform, display, scale, H, V='top'):
        Frame.__init__(self, bounds, platform, display, scalers=scale, Halign=H, Valign=V)
        # def __init__(self, bounds, platform, display, channel, scale, V, H, right_offset=0, colour='white', noflip=1):
        self += SpectrumFrame(self.coords, platform, display, 'left', (0.5, 1.0), V='bottom', H='left' )
        self += SpectrumFrame(self.coords, platform, display, 'right', (0.5, 1.0) , V='bottom', H='right' )
        self.check()
...
class SpectrumStereoFrame(Frame): #""" Horz Split screen - right flipped 'Apple Style' """
    ...
    def __init__(self, bounds, platform, display, scale, H, V='top'):
        ...
        self += SpectrumFrame(self.coords, platform, display, 'right', (1.0, 0.5), V='bottom', H='left', flip=True, led_gap=2, peak_h=0, radius=2, theme='white', barw_min=10, bar_space=0.4)
        self += SpectrumFrame(self.coords, platform, display, 'left', (1.0, 0.5), V='top', H='left', flip=False, led_gap=2, peak_h=0,radius=2, theme='white', barw_min=10, bar_space=0.4)
        self.check()
...
class SpectrumStereoLRFrame(Frame): #""" Horz Split screen - LED Style right flipped  """
    ...
    def __init__(self, bounds, platform, display, scale, H, V='top'):
        ...
        self += SpectrumFrame(self.coords, platform, display, 'right', (1.0, 0.5), V='bottom', H='left', flip=True, led_gap=1, peak_h=1, radius=2, theme='white', barw_min=12 )
        self += SpectrumFrame(self.coords, platform, display, 'left', (1.0, 0.5), V='top', H='left', flip=False, led_gap=1, peak_h=1,radius=2, theme='white', barw_min=12 )
        self.check()
...
class SpectrumStereoSplitFrame(Frame): #""" Horz Split screen - right flipped """
    ...
    def __init__(self, bounds, platform, display, scale, H, V='top'):
        Frame.__init__(self, bounds, platform, display, scalers=scale, Halign=H, Valign=V)
        self += SpectrumFrame(self.coords, platform, display, 'right', (1.0, 0.5), V='bottom', H='left', led_gap=0, flip=True, barw_min=5, bar_space=0.5, tip=True )
        self += SpectrumFrame(self.coords, platform, display, 'left', (1.0, 0.5), V='top', H='left', led_gap=0, barw_min=5, bar_space=0.5, tip=True )
        self.check()

class SpectrumStereoOffsetFrame(Frame):
    def __init__(self, bounds, platform, display, scale, H, V='top'):
        Frame.__init__(self, bounds, platform, display, scalers=scale, Halign=H, Valign=V)
        self += SpectrumFrame(self.coords, platform, display, 'right', (1.0, 1.0), V='bottom', H='left', right_offset=8, barw_min=8, bar_space=1.5, theme='red', led_gap=0, tip=True)
        self += SpectrumFrame(self.coords, platform, display, 'left', (1.0, 1.0), V='bottom', H='left', right_offset=0, barw_min=8, bar_space=1.5, theme='blue', led_gap=0, tip=True )
        self.check()
...
class OscilogrammeBar(Frame):
    """
        ...
        col_mode    - colours change either horizontally across the display, or vertically with the amplitude of the bar
    ...
    def __init__(self, bounds, platform, display, channel, scalers=(1.0, 1.0), Valign='bottom', Halign='left', barsize_pc=1, theme='std', flip=False, \
...
class Oscilogramme(Frame):
    ...
    """
    def __init__(self, bounds, platform, display, channel, scalers=(1.0,1.0), Valign='bottom', Halign='left', theme='std'):
        self.channel = channel
        Frame.__init__(self, platform=platform, bounds=bounds, display=display, scalers=scalers, Valign=Valign, Halign=Halign)
        self.lines   = Line(display, self.coords, circle=False)
...
class Octaviser(Frame, Spectrum):
    def __init__(self, bounds, platform, display, channel, scalers=(1.0,1.0), Valign='bottom', Halign='left', theme='std'):
        ...
        self.arcs = ArcsOctaves(self.display, self.coords, theme='rainbow', NumOcts=self.num_octaves)
...
class CircleModulator(Frame):
    def __init__(self, bounds, platform, display, channel, scalers=(1.0,1.0), Valign='bottom', Halign='left', theme='std'):
        ...
        self.lines   = Line(display, self.coords, circle=True, radius=self.h/2, endstops=(0,2*PI), amp_scale=1.0)
        self.ripples = Line(display, self.coords, circle=True, radius=self.h/2, endstops=(0,2*PI), amp_scale=1.4)
        self.dots    = Dots(display, self.coords, circle=True, radius=self.h/2, endstops=(0,2*PI), amp_scale=0.2)
...
class Diamondiser(Frame, Spectrum):
    BARSPACE = 1
    def __init__(self, bounds, platform, display, channel, scalers=(1.0,1.0), Valign='bottom', Halign='left', theme='std', bar_space=BARSPACE):
        Frame.__init__(self, platform=platform, bounds=bounds, display=display, scalers=scalers, Valign=Valign, Halign=Halign, square=True)
